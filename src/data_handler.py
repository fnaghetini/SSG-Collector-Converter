...
def __create_df_prog_columns(df_prog, year):
    ...
    df_prog['coord_type_code'] = coord_codes['Programada']
...
def __create_df_exec_columns(df_exec, file_name, year):
    ...
    df_exec['coord_type_code'] = coord_codes['Executada']
...
def __generate_surface_samples_prog_template(sample_type, year, df_prog):
...
def __generate_sample_coordinate_prog_template(grid_type, df_prog):
    df_sample_coordinate = df_prog[sample_coordinate_cols]
    df_sample_coordinate['GRID_TYPE_CODE'] = grid_type
    return df_sample_coordinate.sort_values(by=sample_coordinate_pk)


def __generate_surface_samples_locd_exec_template(sample_type, year, df_prog, df_exec):
    df_surface_samples = df_prog[surface_samples_core_cols]
